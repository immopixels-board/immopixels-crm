...
export default function TeamChat({ supabase, currentUser, staff, onClose, aiOpen }) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 64, right: aiOpen ? 412 : 20, width: 560, maxWidth: 'calc(100vw - 40px)', height: 'min(70vh, 600px)', maxHeight: 'calc(100vh - 140px)', background: 'var(--bg2)', border: '0.5px solid var(--border)', borderRadius: 16, boxShadow: '0 12px 40px rgba(120,90,30,.16)', zIndex: 300, display: 'flex', flexDirection: 'column', overflow: 'hidden', transition: 'right .22s cubic-bezier(.4,0,.2,1)' }}>
              ...
              return (
                <div key={msg.id} className="chat-msg" style={{ display: 'flex', gap: 7, alignItems: 'flex-end', flexDirection: isMe ? 'row-reverse' : 'row' }}>
                  ...
                  <div style={{ maxWidth: '75%' }}>
                    ...
                    <div style={{ background: isMe ? '#6b6b6e' : 'var(--bg3)', color: isMe ? '#fff' : 'var(--t1)', borderRadius: isMe ? '13px 13px 4px 13px' : '13px 13px 13px 4px', padding: '8px 12px', fontSize: 12, lineHeight: 1.5, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <input ref={inputRef} value={input}
                onChange={e => {
                  ...
                  if (match) { const q = match[1].toLowerCase(); const allOpt = 'all'.startsWith(q) ? [{ id:'__all__', name:'all', init:'ALL', color:'#6b6b6e', avatar_url:null }] : []; setMentionList([...allOpt, ...staff.filter(s => s.name.toLowerCase().includes(q) || s.init.toLowerCase().includes(q))].slice(0,7)); setMentionIdx(0) }
                  else setMentionList([])
                }}
                onKeyDown={e => {
