                        ...
                        <div className="bcol-cards" data-colid={col.id} style={{ overflowY: 'auto', padding: '14px 8px 8px 8px', display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
                          {(() => {
                            ...
                            colCards.forEach((card, i) => {
                              ...
                              items.push(
                                <CardItem key={card.id} card={card} compact={compactEnabled && compactCards.includes(card.id)} staff={staff} onlineUsers={onlineUsers} fontSize={fontSize} dragging={dragging} clients={clients} border={cardBorder(card)} overdueDays={cardOverdueDays(card)} overdueBg={cardOverdueBg(card)} onNoteChange={onNoteChange} onNoteEnter={onNoteEnter} onClick={() => setActiveCard(card)} onDragStart={(e, cardEl) => startCustomDrag(e.clientX || e.touches?.[0]?.clientX, e.clientY || e.touches?.[0]?.clientY, cardEl, card)} onSend={openSend} droppedId={droppedCard}
                            onCheck={async (card) => {
...
<form onSubmit={e => { e.preventDefault(); if(isAdminOrSub) saveStaffFn(new FormData(e.target)) }} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 13, padding: 22, width: 420, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.12)' }}>
              ...
              <div style={{ marginBottom: 9 }}><label style={LS}>Adresse (Heimadresse)</label><input name="address" defaultValue={editStaff?.address || ''} placeholder="Straße, PLZ Stadt" style={IS} /></div>
              <div style={{ marginBottom: 14 }}>
                <label style={LS}>Farbe</label>
