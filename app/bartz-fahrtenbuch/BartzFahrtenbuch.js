...
export default function BartzFahrtenbuch() {
  ...
  function exportExcel() {
    const th = HEAD.map(h => `<th style="background:#b8892a;color:#fff;padding:6px;border:1px solid #ccc">${h}</th>`).join('')
  ...
  function exportPDF() {
    ...
    w.document.write(`<html><head><title>${expName()}</title><style>
      ...
      th{background:#b8892a;color:#fff;padding:6px 7px;text-align:left;white-space:nowrap}
      ...
      tfoot td{font-weight:bold;border-top:2px solid #b8892a}
    </style></head><body>
