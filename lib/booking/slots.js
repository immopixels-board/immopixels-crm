...
async function getGCalToken() {
  // Always use CD's token (the main GCal account)
  const { data } = await sb()
  ...
  if (!data) {
    console.warn('[slots] gcal_tokens: no row found for CD')
    return null
  }

  console.log('[slots] token row:', { has_rt: !!data.refresh_token, rt_len: data.refresh_token?.length, expires_at: data.expires_at })
...
async function busyIntervals(staffInit, dateStr) {
  // Read ALL cards (GCal-synced + manual) from DB — no direct Google API call
  const { data } = await sb()
    .from('cards')
