...
export async function GET() {
  ...
  if (!token) return Response.json({ ok: false, reason: 'no token - bitte erst Google verbinden' })

  const webhookUrl = 'https://immopixels-crm.vercel.app/api/gcal/webhook'
  const results = []
    ...
    try {
      ...
      const r = await fetch(
        `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(cal.id)}/events/watch`,
        {
          ...
          body: JSON.stringify({ id: channelId, type: 'web_hook', address: webhookUrl, expiration: expiry.toString() })
        }
      )
      const data = await r.json()
      results.push({ cal: cal.init, ok: r.ok, error: r.ok ? null : (data.error?.message || JSON.stringify(data)) })
      if (r.ok && data.id) {
        await supabase.from('settings').upsert({ key: `gcal_watch_${cal.init}`, value: JSON.stringify({ channelId: data.id, resourceId: data.resourceId, expiry }) }, { onConflict: 'key' })
      }
    } catch (e) { results.push({ cal: cal.init, ok: false, error: e.message }) }
  }
