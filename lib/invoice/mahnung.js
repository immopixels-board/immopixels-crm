...
export async function generateMahnungPdf({ inv, seller, template = {}, text, gebuehr = 5, stufe = 1 }) {
  ...
  const cwrap = (txt, f, size, mw) => { const words = String(txt ?? '').split(/\s+/).filter(Boolean); const lines = []; let cur = ''; for (const w of words) { const t = cur ? cur + ' ' + w : w; if (cur && f.widthOfTextAtSize(t, size) > mw) { lines.push(cur); cur = w } else cur = t } if (cur) lines.push(cur); return lines.length ? lines : [''] }
  ...
  cwrap(b.company || inv.client_name || '', B, 11, 300).forEach(ln => { T(ln, L, y, 11, B, dark); y -= 13 })
  if (b.contact) { T(b.contact, L, y, 9, F, dark); y -= 12 }
  ;(b.address || '').split(/[\n,]/).map(s => s.trim()).filter(Boolean).forEach(line => { T(line, L, y, 9, F, dark); y -= 12 })
  y -= 16
