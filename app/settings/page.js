  ...
  return (
    <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18, marginBottom:14 }}>
      <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
        <i className={'ti ' + icon} style={{ fontSize:16, color:'#6b6b6e' }} />{title}
      </div>
          ...
          return (
            <div key={idx} style={{ display:'flex', alignItems:'center', gap:4, background:'#f4f2ef', border:'0.5px solid #eeeae6', borderRadius:20, padding:'5px 8px 5px 12px', fontSize:12, fontWeight:600, color:'#1c1a16' }}>
              ...
              <button onClick={()=>{setEditIdx(idx);setEditVal(lbl)}} style={{ background:'none', border:'none', cursor:'pointer', color:'#aaa8a0', padding:'0 3px', display:'flex', alignItems:'center' }}
                onMouseEnter={e=>e.currentTarget.style.color='#6b6b6e'} onMouseLeave={e=>e.currentTarget.style.color='#aaa8a0'}>
      ...
      <div style={{ display:'flex', gap:7 }}>
        <input value={newVal} onChange={e=>setNewVal(e.target.value)}
          ...
          onFocus={e=>e.target.style.borderColor='#6b6b6e'} onBlur={e=>e.target.style.borderColor='#ddd9d2'}/>
        <button onClick={()=>{if(newVal.trim()){onAdd(newVal.trim());setNewVal('')}}}
          style={{ background:'#6b6b6e', color:'#fff', border:'none', borderRadius:8, padding:'8px 14px', fontSize:12, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:5 }}>
  ...
  return (
    <div>
      <button onClick={activate} disabled={loading}
        style={{ background:'#6b6b6e', color:'#fff', border:'none', borderRadius:8, padding:'9px 18px', fontSize:13, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:6, opacity:loading?0.7:1 }}>
...
export default function Settings() {
  ...
  const NAV = [
    { key:'appearance', label:'Aussehen', icon:'ti-palette', color:'#6b6b6e', bg:'rgba(184,137,42,.12)' },
        ...
        <div style={{ display:'flex', gap:18 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            {activeNav === 'appearance' && (
              <>
                ...
                <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18, marginBottom:14 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
                    <i className="ti ti-text-size" style={{ fontSize:16, color:'#6b6b6e' }} /> Schriftgröße
                  </div>
                  <div style={{ display:'flex', gap:8 }}>
                    {FONT_SIZES.map(f => (
                      <div key={f.key} onClick={() => { setFontSize(f.key); saveUserSetting('font_size', f.key) }}
                        style={{ flex:1, background: fontSize===f.key ? 'rgba(184,137,42,.08)' : '#f4f2ef', border: fontSize===f.key ? '1.5px solid #6b6b6e' : '0.5px solid #eeeae6', borderRadius:9, padding:'12px', textAlign:'center', cursor:'pointer', transition:'all .15s' }}>
                        <div style={{ fontSize: f.key==='sm'?'13px':f.key==='lg'?'18px':'15px', fontWeight:700, color: fontSize===f.key ? '#6b6b6e' : '#1c1a16', marginBottom:4 }}>Aa</div>
                        <div style={{ fontSize:11, color: fontSize===f.key ? '#6b6b6e' : '#8a8278', fontWeight:600 }}>{f.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18, marginBottom:14 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
                    <i className="ti ti-layout-cards" style={{ fontSize:16, color:'#6b6b6e' }} /> Kartengröße
                  </div>
                  <div style={{ display:'flex', gap:10, alignItems:'flex-end' }}>
                    {CARD_SIZES.map(cs => (
                      <div key={cs.key} onClick={() => { setCardSize(cs.key); saveUserSetting('card_size', cs.key) }} style={{ cursor:'pointer', textAlign:'center' }}>
                        <div style={{ width:cs.w, border: cardSize===cs.key ? '1.5px solid #6b6b6e' : '0.5px solid #ddd9d2', borderRadius:9, padding: cs.key==='compact'?'7px 9px':cs.key==='standard'?'9px 10px':'12px', background: cardSize===cs.key ? 'rgba(184,137,42,.05)' : '#fff', boxShadow: cardSize===cs.key ? '0 0 0 3px rgba(184,137,42,.1)' : 'none', transition:'all .15s' }}>
                        ...
                        <div style={{ fontSize:11, fontWeight:700, color: cardSize===cs.key ? '#6b6b6e' : '#4a4540', marginTop:6 }}>{cs.label}</div>
                ...
                <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
                    <i className="ti ti-paint" style={{ fontSize:16, color:'#6b6b6e' }} /> Hintergrund
                  </div>
                  ...
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:16 }}>
                    {BG_OPTIONS.map(bg => (
                      <div key={bg.key}
                        ...
                        title={bg.label}
                        style={{ width:40, height:40, borderRadius:9, background:bg.color, cursor:'pointer', border: bgColor===bg.key && !bgImage ? '2px solid #6b6b6e' : '1px solid #ddd9d2', boxShadow: bgColor===bg.key && !bgImage ? '0 0 0 3px rgba(184,137,42,.2)' : 'none', transition:'all .15s', opacity: bgImage ? 0.45 : 1 }}
                  ...
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                    <div onClick={()=>{ setBgImage(null); saveBgImage(null) }}
                      style={{ width:56, height:40, borderRadius:7, background:'var(--bg3)', border: !bgImage?'2px solid #6b6b6e':'1px solid #ddd9d2', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, color:'#8a8278', transition:'all .15s' }}>
                      Keine
                    </div>
                    {BG_IMAGE_OPTIONS.map(bg => (
                      <div key={bg.key} onClick={()=>saveBgImage(bg.src)} title={bg.label}
                        style={{ width:56, height:40, borderRadius:7, backgroundImage:'url('+bg.src+')', backgroundSize:'cover', cursor:'pointer', border: bgImage===bg.src?'2px solid #6b6b6e':'1px solid #ddd9d2', boxShadow: bgImage===bg.src?'0 0 0 3px rgba(184,137,42,.2)':'none', transition:'all .15s', position:'relative' }}
                        onMouseEnter={e=>e.currentTarget.style.transform='scale(1.05)'}
                        onMouseLeave={e=>e.currentTarget.style.transform='none'}>
                        {bgImage===bg.src && <div style={{ position:'absolute', top:2, right:2, width:14, height:14, borderRadius:'50%', background:'#6b6b6e', display:'flex', alignItems:'center', justifyContent:'center' }}><i className="ti ti-check" style={{fontSize:9,color:'#fff'}}/></div>}
            ...
            {activeNav === 'appearance' && (
              <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18, marginBottom:14 }}>
                <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
                  <i className="ti ti-columns" style={{ fontSize:16, color:'#6b6b6e' }} />
                  Spalten-Header Stil
                </div>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  ...
                  <div onClick={()=>{ const nv=!colWidgetOn; setColWidgetOn(nv); saveUserSetting('col_widget_header',nv) }}
                    style={{ width:44, height:24, borderRadius:12, background:colWidgetOn?'#6b6b6e':'#ddd9d2', cursor:'pointer', position:'relative', transition:'background .2s', flexShrink:0 }}>
            ...
            {activeNav === 'appearance' && (
              <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18, marginBottom:14 }}>
                <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:14 }}>
                  <i className="ti ti-layout-list" style={{ fontSize:16, color:'#6b6b6e' }} />
                  Kompakt Karten
                </div>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div>
                  ...
                  <div onClick={()=>{ const newVal=!compactCardsOn; setCompactCardsOn(newVal); saveUserSetting('compact_cards_enabled',newVal) }}
                    style={{ width:44, height:24, borderRadius:12, background:compactCardsOn?'#6b6b6e':'#ddd9d2', cursor:'pointer', position:'relative', transition:'background .2s', flexShrink:0 }}>
          ...
          {activeNav === 'integrations' && (
            <div>
              ...
              <div style={{ background:'#fff', border:'0.5px solid #eeeae6', borderRadius:12, padding:18, marginBottom:14 }}>
                <div style={{ fontSize:14, fontWeight:700, color:'#1c1a16', display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
                  <i className="ti ti-photo" style={{ fontSize:16, color:'#6b6b6e' }} />
                  Leistungs-Bilder
                </div>
                <div style={{ fontSize:12, color:'#8a8278', marginBottom:14, lineHeight:1.6 }}>
                  Runde Kategorie-Bilder für die Buchungsseite hochladen und verwalten.
                </div>
                <a href="/admin/leistungen" style={{ display:'inline-block', padding:'9px 18px', fontSize:13, fontWeight:700, background:'#6b6b6e', color:'#fff', borderRadius:8, textDecoration:'none' }}>Bilder verwalten →</a>
              </div>
            </div>
          )}
          </div>
