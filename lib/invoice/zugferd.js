...
export async function generateZugferdPdf({ inv, items, seller, template = {} }) {
  ...
  const kmSum = (items || []).reduce((s, it) => { const m = String(it.description || '').match(/([\d.,]+)\s*km/i); if (!m) return s; const n = parseFloat(m[1].replace(/\./g, '').replace(',', '.')); return s + (isNaN(n) ? 0 : n) }, 0)
