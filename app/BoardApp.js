...
function MaklerTermineModal({ cols, clients, onClose, onDone }) {
  ...
  React.useEffect(() => { (async () => {
    try { const r = await fetch('/api/maklertermine'); const j = await r.json(); if (!j.ok) throw new Error(j.error); setCals(j.calendars || []); if (j.fallback) setWarn('Volle Kalenderliste nicht verfügbar (alte Google-Verbindung). Im Kalender-Tab „Mit Google verbinden" neu ausführen — oder unten die Kalender-ID manuell eintragen.'); if (j.calendars?.[0]) setCalId(j.calendars[0].id) }
  ...
  async function call(create) {
    ...
    try {
      const r = await fetch('/api/maklertermine', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ calendarId: (manualId.trim() || calId), year, clientName: create ? clientVal : undefined, columnId: create ? colId : undefined, create }) })
      const j = await r.json(); if (!j.ok) throw new Error(j.error)
      if (create) { setResult(j); onDone && onDone() } else setPreview(j)
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '50px 16px', zIndex: 200, overflowY: 'auto' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: 22, width: '100%', maxWidth: 560, boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--t3)', marginBottom: 16 }}>Termine aus einem deiner Google-Kalender als echte CRM-Karten zum gewählten Kunden anlegen.</div>
        ...
        {cals === null ? <div style={{ color: 'var(--t3)', fontSize: 13, padding: '10px 0' }}>Kalender werden geladen…</div> : (
          <>
            <div style={{ marginBottom: 12 }}><label style={lbl}>Google-Kalender</label>
              <select value={calId} onChange={e => { setCalId(e.target.value); setPreview(null) }} style={inp}>{(cals || []).map(c => <option key={c.id} value={c.id}>{c.summary}{c.primary ? ' (Hauptkalender)' : ''}</option>)}</select>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              ...
              <button onClick={() => { if (!clientVal || !colId) { setErr('Kunde und Spalte wählen.'); return } if (confirm('Termine als Karten importieren?')) call(true) }} disabled={busy || !preview} style={gbtn}>{busy ? 'Importiere…' : 'Importieren'}</button>
            </div>
            {preview && !result && <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 9, padding: 12, marginTop: 6 }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>{preview.count} Termin(e) in {year} gefunden{preview.count > (preview.events?.length || 0) ? ' (Vorschau: erste ' + preview.events.length + ')' : ''}:</div>
              <div style={{ maxHeight: 200, overflowY: 'auto', fontSize: 12 }}>{(preview.events || []).slice(0, 60).map((e, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '4px 0', borderBottom: '0.5px solid var(--border)' }}>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--t3)', marginTop: 8 }}>Beim Import werden bereits vorhandene Karten (gleicher Kunde + Datum + Titel) übersprungen.</div>
            </div>}
...
export default function Home() {
  ...
  // Demo-Modus: Dirk Fellhauer sehol ne jelenjen meg
  const HIDDEN_FOR_DEMO = s => ((s.email || '').toLowerCase() === 'd.fellhauer@immopixels.de') || /dirk\s*fell?hauer/i.test(s.name || '')
  function isDemoUser(list) {
    if (!currentUser) return false
    const meRow = (list || staff).find(s => s.email === currentUser.email)
    return meRow?.role_level === 'demo'
  }
  async function loadStaff() {
    const { data } = await supabase.from('staff').select('*').order('name')
    let list = data || []
    if (isDemoUser(list)) list = list.filter(s => !HIDDEN_FOR_DEMO(s))
    setStaff(list)
  }
  ...
  async function loadPhonebook() {
    ...
    if (data) setPhonebook(isDemoUser() ? data.filter(p => !HIDDEN_FOR_DEMO(p)) : data)
  }
  ...
  useEffect(() => { if (currentUser) { loadStaff(); loadPhonebook() } }, [currentUser?.email]) // demo-szűrés újraalkalmazása login után
                  ...
                  <tbody>
                    {vis.map(cl => { const sel = selClients.has(cl.id); const count = cards.filter(card => card.client_name === cl.name || card.client_name === cl.short_name).length; return (
                      <tr key={cl.id} style={{ borderBottom: '0.5px solid var(--border)', background: sel ? 'var(--gdbg)' : 'transparent', cursor: 'pointer' }} onClick={(e) => { if (e.target.type === 'checkbox') return; setEditClient(cl); setClientColorState(cl.color || ''); setClientSpJson(JSON.stringify(cl?.service_prices || {})); setModal('client') }}>
                        ...
                        <td style={{ padding: '8px 10px', fontWeight: 700 }}>{cl.name}</td>
