...
export default function Home() {
  ...
  async function loadBoards() {
    ...
    if (!activeBoardId && data && data.length && currentUser?.email) {
      ...
      const { data: meRow } = await supabase.from('staff').select('id,email,role_level,allowed_boards,init').eq('email', currentUser.email).maybeSingle()
      setMyRow(meRow)
      const allowed = meRow?.allowed_boards || []
      const isAdmin = meRow?.role_level === 'admin'
      // Admin mindig mindet látja, ha nincs konkrét beállítása.
      // Más usernél: ha üres az allowed_boards, NEM állítunk be semmit (= nem lát semmit)
      if (isAdmin && !allowed.length) {
        setActiveBoardId(data[0].id)
      } else if (allowed.length) {
        const first = data.find(b => allowed.includes(b.id))
        if (first) setActiveBoardId(first.id)
        // ha nincs egyezés (törölt board?), nem állítunk semmit
      }
      // nem admin + üres allowed → semmit nem lát (activeBoardId marad null → loadCols üres)
    } else if (currentUser?.email && !myRow) {
      // ha valami miatt nem futott be a myRow setup, próbáljuk meg most
      const { data: meRow } = await supabase.from('staff').select('id,email,role_level,allowed_boards,init').eq('email', currentUser.email).maybeSingle()
      setMyRow(meRow)
    }
  }
      ...
      {tab === 'board' && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ height: 42, background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 6, padding: '0 14px', flexShrink: 0 }}>
            {(() => {
              // amíg betölt, ne mutassunk semmit (különben átmenetileg "Kein Board" jelenne meg)
              if (!myRow) return null
              const allowed = myRow.allowed_boards || []
              const isAdmin = myRow.role_level === 'admin'
              // Admin nélküli konfigot mindet látja. Más user CSAK az allowed_boards-ot.
              const visibleBoards = (isAdmin && !allowed.length)
                ? boards
                : boards.filter(b => allowed.includes(b.id))
              if (visibleBoards.length === 0) {
                return <span style={{ fontSize: 12, color: 'var(--t3)' }}>Kein Board zugewiesen</span>
              }
