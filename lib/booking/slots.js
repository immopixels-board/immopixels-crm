...
export async function getDaySlots(serviceId, dateStr, addonMin = 0, excludeGcalId = null, shootAddress = null) {
  ...
  if (shootAddress && travelMod) {
    ...
    const addrByInit = Object.fromEntries((stAddr || []).map(s => [s.init, s.address || null]))
    for (const p of providers) {
      const ha = addrByInit[p.staff_init]
  ...
  for (const p of providers) {
    ...
    for (const w of wd) {
      ...
      for (let t = wStart; t + dur <= wEnd && t <= LAST_SLOT; t += SLOT_STEP_MIN) {
        ...
        // utazás-tudatosság: ha a meglévő terminről nem érne oda időben (vagy az új
        // terminről a következő meglévőre késne), a fotós erre a slotra NEM elérhető.
        // Ha befér, de szűkösen (útidő + puffer), csak "knapp" jelzést kap.
        let tight = false, blocked = false
        if (shootAddress) {
          // előző esemény (ami t előtt ér véget) és következő (ami t+dur után kezdődik)
          ...
          if (prev?.location) {
            ...
            if (to != null) {
              const gap = t - prev.end
              if (gap < to) blocked = true
              else if (gap < to + TRAVEL_BUFFER_MIN) tight = true
            }
          }
        ...
        const hhmm = toHHMM(t)
        if (!slotMap[hhmm]) slotMap[hhmm] = { providers: new Set(), tightAll: true }
