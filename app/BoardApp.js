...
var AUTO_CL = ['Fotografiert', 'Für Bearbeitung gesendet', 'In Bearbeitung', 'Rausgeschickt']
function getAutoChecklist(cardType) {
  const base = ['Fotografiert', 'Für Bearbeitung gesendet', 'In Bearbeitung', 'Rausgeschickt']
...
export default function Home() {
  ...
  async function loadMonthly(month) {
    ...
    const cards = (data || []).filter(c => c.card_type !== 'todo' && (c.client_name || '').trim())
    ...
    for (const c of cards) {
      ...
      const key = cl ? 'id:' + cl.id : 'raw:' + nrm(c.client_name)
      if (!groups.has(key)) groups.set(key, { name: cl?.name || c.client_name, client: cl, cards: [] })
      groups.get(key).cards.push(c)
    }
    const list = [...groups.values()].sort((a, b) => a.name.localeCompare(b.name, 'de'))
    setMonthGroups(list); setMonthChecked(new Set(cards.map(c => c.id))); setMonthLoading(false)
  }
  ...
  function monthlyCreateForClient(group) {
    ...
    const types = getTypes(photoCats)
    const items = checked.map(c => {
      const label = findType(types, c.card_type)?.l || (c.card_type || 'Foto')
      const addr = c.booking_address || c.title || ''
      const dd = c.card_date ? c.card_date.split('-').reverse().join('.') : ''
      return { title: `${label}${addr ? ' – ' + addr : ''}${dd ? ' (' + dd + ')' : ''}`, quantity: 1, unit_price: c.price > 0 ? c.price : '' }
    })
    openInvoice(group.client, items)
  }
        ...
        for (const tool of toolUses) {
          ...
          try {
            if (tool.name === 'create_card') {
              ...
              if (card) {
                // Auto checklist
                for (const text of ['Fotografiert','Für Bearbeitung gesendet','In Bearbeitung','Rausgeschickt']) {
                  await supabase.from('checklist_items').insert({ card_id: card.id, text, done: false })
                }
              ...
              {(() => {
                ...
                return (
                  <div key={col.id} className="bcol"
                    draggable={false}
                    style={{ width: isCollapsed ? 44 : 272, minWidth: isCollapsed ? 44 : 272, background: getColStyle(col.color).bg, border: '1px solid var(--border)', borderRadius: 11, display: 'flex', flexDirection: 'column', maxHeight: 'calc(100vh - 116px)', flexShrink: 0, transition: 'all .2s', cursor: isCollapsed ? 'pointer' : 'default', opacity: isColDragged ? 0.8 : 1 }}
                    data-colid={col.id}
>
                    {isCollapsed ? (
      ...
      {showWidgets && widgets.map(w => {
        ...
        return (
          <div key={w.id}
            ...
            <div style={{ padding:'9px 12px', background: w.type==='weather' ? 'linear-gradient(135deg,#9fd5df,#7BBFCB)' : w.type==='todo' ? 'linear-gradient(135deg,#dcc088,#C9A96E)' : w.type==='nexttermin' ? 'linear-gradient(135deg,#b8cca8,#9CAF88)' : w.type==='stats' ? 'linear-gradient(135deg,#ffd4b0,#FFBE98)' : 'linear-gradient(135deg,#c49a75,#A67B5B)', borderBottom:'none', display:'flex', alignItems:'center', gap:7, cursor:'grab', opacity: minimizedWidgets.includes(w.id) ? 0.5 : 1, transition:'opacity .2s' }}>
                        ...
                        <div key={gi} style={{ border: '0.5px solid var(--border)', borderRadius: 10, marginBottom: 10, overflow: 'hidden' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '9px 12px', background: 'var(--bg3)' }}>
                            <div style={{ minWidth: 0 }}>
                              ...
                              {!linked && <div style={{ fontSize: 10, color: '#b91c1c', fontWeight: 600 }}>⚠ nicht mit Billomat verknüpft</div>}
                            </div>
              ...
              {invResult ? (
                ...
                <>
                  ...
                  <div style={{ flex: 1, overflowY: 'auto', minHeight: 60 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 64px 96px 30px', gap: 6, fontSize: 10, fontWeight: 700, color: 'var(--t3)', padding: '0 2px 4px' }}>
                      <span>Bezeichnung</span><span>Menge</span><span>Einzelpr. €</span><span></span>
                    </div>
                    {invItems.map((it, i) => (
                      <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 64px 96px 30px', gap: 6, marginBottom: 6 }}>
                        <input value={it.title || ''} onChange={e => invSetItem(i, 'title', e.target.value)} placeholder="z.B. Immobilienfotos" style={{ ...IS, padding: '7px 9px' }} />
                        <input value={it.quantity ?? 1} onChange={e => invSetItem(i, 'quantity', e.target.value)} inputMode="decimal" style={{ ...IS, padding: '7px 9px' }} />
                        <input value={it.unit_price ?? ''} onChange={e => invSetItem(i, 'unit_price', e.target.value)} inputMode="decimal" placeholder="0.00" style={{ ...IS, padding: '7px 9px' }} />
...
function CardItem({ card, staff, border, overdueDays = 0, overdueBg, onNoteChange, onNoteEnter, onClick, onDragStart, onSend, onCheck, onDelete, onColorChange, droppedId, noteMention, setNoteMention, dirtyCards, editingCards, onMoveUp, onMoveDown, onlineUsers, fontSize = 14, dragging = null, clients = [], allTypes = null, hideNotSentBadge = false, compact = false, onCompactToggle = null }) {
  ...
  return (
    <div
      ...
      {showNotSent && (
        <div style={{ position:'absolute', top: showOverdue ? 8 : -9, left:10, background:'#fff7ed', border:'0.5px solid #fed7aa', color:'#c2410c', fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:10, display:'flex', alignItems:'center', gap:3, zIndex:5, pointerEvents:'none' }}>
          <i className="ti ti-send" style={{ fontSize:9 }}></i>
          Noch nicht gesendet
        </div>
      )}
      {showReel && (
        <div style={{ position:'absolute', top: showOverdue ? 8 : -9, left:10, background:'#fefce8', border:'0.5px solid #fde047', color:'#854d0e', fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:10, display:'flex', alignItems:'center', gap:3, zIndex:5, pointerEvents:'none' }}>
