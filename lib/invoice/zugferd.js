...
export async function generateZugferdPdf({ inv: invIn, items, seller, template = {}, clientKmTotal = 0 }) {
  ...
  if (inv.clientKmTotal > 0) { T('Für Ihre Aufträge gefahren (gesamt): ' + Number(inv.clientKmTotal).toLocaleString('de-DE') + ' km — überwiegend nicht berechnet.', L, y, 8, F, mut); y -= 13 }
