...
// TYPES is built purely from photoCats (single source of truth)
var TYPES = {}

function getTypes(photoCats) {
  const result = {}
  if (photoCats && photoCats.length) {
    ...
    // fallback defaults
    result['foto']      = { i:'', l:'Foto',        c:'#b8892a', bg:'#b8892a22', br:'#b8892a55' }
    result['fotoreel']  = { i:'', l:'Foto+Reel',   c:'#6d28d9', bg:'#6d28d922', br:'#6d28d955' }
    result['fotodrohn'] = { i:'', l:'Foto+Drohne', c:'#a16207', bg:'#a1620722', br:'#a1620755' }
    result['drohne']    = { i:'', l:'Drohne',      c:'#15803d', bg:'#15803d22', br:'#15803d55' }
    result['reel']      = { i:'', l:'Reel',        c:'#6d28d9', bg:'#6d28d922', br:'#6d28d955' }
    result['360']       = { i:'', l:'360°',        c:'#0891b2', bg:'#0891b222', br:'#0891b255' }
  }
...
export default function Home() {
  ...
  async function loadSettings() {
    ...
    if (s.photo_categories) {
      const cats = s.photo_categories.map(c => typeof c === 'string' ? { l:c, c:'#b8892a' } : { l:c.l, c:c.c||'#b8892a' })
      setPhotoCats(cats)
    }
  ...
  function renameCol(colId, title) {
    const col = cols.find(c => c.id === colId)
    if (col) setColModal(col)
  }
...
>
                  ...
                  <div key={col.id} style={{ marginBottom: 18 }}>
                    ...
                    {cardsForCol(col.id).map(card => (
                      <div key={card.id} onClick={() => setActiveCard(card)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, marginBottom: 4, cursor: 'pointer' }}>
                        <span style={{ fontSize: 14 }}>{card.is_todo ? '✓' : (findType(allTypes||getTypes([]),card.card_type)||{i:''}).i}</span>
                        <div style={{ flex: 1 }}>
          ...
          <div style={{ width: 'clamp(180px, 22vw, 260px)', flexShrink: 0, borderRight: '1px solid var(--border)', background: 'var(--bg2)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
              {cards.filter(c => c.card_date === new Date().toISOString().slice(0, 10)).map(card => {
                const t = findType(allTypes||getTypes([]), card.card_type) || { i:'', l:'Foto', c:'#b8892a', bg:'#b8892a22', br:'#b8892a55' }
                return (
      ...
      {activeCard && (
        ...
        <ColumnModal col={colModal} onClose={() => setColModal(null)} isAdmin={me?.role_level==='admin' || me?.role_level==='subadmin'}
          onSave={async (name, colorKey, privateCol, dotColor) => {
            const vtr = privateCol ? ['admin'] : []
            const colData = { title: name, color: colorKey || null, visible_to_roles: vtr }
            if (dotColor) colData.dot_color = dotColor
            if (colModal.id) {
              await supabase.from('columns').update(colData).eq('id', colModal.id)
            } else {
              const { data: mx } = await supabase.from('columns').select('position').order('position', { ascending: false }).limit(1)
              await supabase.from('columns').insert({ ...colData, position: (mx?.[0]?.position || 0) + 1 })
            }
    ...
    <div
      ...
      <div
        ...
        <i className="ti ti-grip-vertical"
          style={{ fontSize:12, color:'var(--t3)', flexShrink:0, cursor:'grab' }}
      ...
      {(() => { const cl = clients.find(c => c.id === card.client_id); return cl?.extra_link ? <a href={cl.extra_link} target="_blank" rel="noopener" onClick={e=>e.stopPropagation()} style={{ fontSize:10, color:'var(--gold)', display:'flex', alignItems:'center', gap:3, marginBottom:2, textDecoration:'none' }}><i className="ti ti-link" style={{ fontSize:10 }}></i>{cl.extra_link.replace(/^https?:\/\/(www\.)?/,'').slice(0,30)+'…'}</a> : null })()}
