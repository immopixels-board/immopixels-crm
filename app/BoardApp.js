...
export default function Home() {
  ...
  function guessClientName(title) {
    ...
    if (!t) return ''
    let m = clients.find(c => (c.short_name && t.includes(c.short_name.toLowerCase())) || (c.name && t.includes(c.name.toLowerCase())))
    if (!m) m = clients.find(c => { const w = (c.name || '').trim().split(/\s+/)[0]; return w && w.length >= 3 && t.includes(w.toLowerCase()) })
    if (!m) m = clients.find(c => c.short_name && c.short_name.toLowerCase().split(/\s+/).some(w => w.length > 2 && t.includes(w)))
