...
function hexAlpha(hex, a) {
  // append alpha to hex: '#b8892a' + '22' → '#b8892a22'
...
function getTypes(photoCats) {
  ...
  const cats = (photoCats && photoCats.length) ? photoCats : defaults
  for (const cat of cats) {
    const obj = typeof cat === 'string' ? { l:cat, c:'#b8892a' } : cat
    const key = obj.l.toLowerCase().replace(/[^a-z0-9]/g,'')
    if (key && !result[key]) {
      const c = obj.c || '#b8892a'
      result[key] = { i:'', l:obj.l, c, bg:hexAlpha(c,'22'), br:hexAlpha(c,'66') }
    }
  }
  result['todo'] = { i:'', l:'To Do', c:'#6d28d9', bg:'#6d28d922', br:'#6d28d966' }
  return result
}
