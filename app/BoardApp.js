...
export default function Home() {
  ...
  async function loadBoards() {
    ...
    setBoards(data || [])
    // myRow betöltése — a meRow-t LOKÁLISAN használjuk (nem várjuk meg a setMyRow state-frissítését)
    let meRow = null
    if (currentUser?.email) {
      const { data: row } = await supabase.from('staff').select('id,email,role_level,allowed_boards,init').eq('email', currentUser.email).maybeSingle()
      meRow = row
      setMyRow(row)
    }
    // aktív board választás
    if (!activeBoardId && data && data.length) {
      const isAdmin = meRow?.role_level === 'admin' || meRow?.role_level === 'subadmin'
      if (isAdmin) {
        setActiveBoardId(data[0].id) // admin: első board
      } else {
        ...
        // nem admin + üres allowed → activeBoardId marad null → loadCols üres (semmi nem látszik)
      }
    }
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ height: 42, background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 6, padding: '0 14px', flexShrink: 0 }}>
            {(() => {
              ...
              if (!myRow) return null // amíg betölt, ne villantsunk semmit
              const isAdmin = myRow.role_level === 'admin' || myRow.role_level === 'subadmin'
              const allowed = myRow.allowed_boards || []
              const visibleBoards = isAdmin ? boards : boards.filter(b => allowed.includes(b.id))
