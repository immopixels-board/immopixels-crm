...
function MaklerTermineModal({ cols, clients, onClose, onDone }) {
  ...
  React.useEffect(() => { (async () => {
    try { const r = await fetch('/api/maklertermine'); const j = await r.json(); if (!j.ok) throw new Error(j.error); setCals(j.calendars || []); if (j.calendars?.[0]) setCalId(j.calendars[0].id) }
  ...
  async function call(create) {
    ...
    try {
      const r = await fetch('/api/maklertermine', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ calendarId: calId, year, clientName: create ? clientVal : undefined, columnId: create ? colId : undefined, create }) })
        ...
        {cals === null ? <div style={{ color: 'var(--t3)', fontSize: 13, padding: '10px 0' }}>Kalender werden geladen…</div> : (
          <>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              <button onClick={() => call(false)} disabled={busy || !calId} style={ghost}>{busy && !result ? '…' : 'Vorschau laden'}</button>
              <div style={{ flex: 1 }} />
