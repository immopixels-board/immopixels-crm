...
export default function Home() {
  ...
  async function loadAll() {
    setLoading(true)
    await Promise.all([loadCols(), loadCards(), loadStaff(), loadBoards(), loadClients(), loadMaklers(), loadSettings(), loadServicePrices(), loadPhonebook()])
    // Check birthdays
    const today = new Date()
    const mm = String(today.getMonth()+1).padStart(2,'0')
    const dd = String(today.getDate()).padStart(2,'0')
  ...
  async function loadCols(boardIdOverride) {
    ...
    if (data) {
      ...
      const filterBoardId = boardIdOverride !== undefined ? boardIdOverride : activeBoardId
      const visible = data.filter(col =>
        (!col.visible_to_roles || col.visible_to_roles.length === 0 || col.visible_to_roles.includes(myRole))
        && (!filterBoardId || !col.board_id || col.board_id === filterBoardId)
      )
      setCols(visible)
    }
  }
  ...
  async function loadBoards() {
    ...
    setBoards(data || [])
    // ha még nincs aktív board, az első elérhetőt választjuk amihez van hozzáférés
    if (!activeBoardId && data && data.length && currentUser?.email) {
      // friss staff sor a DB-ből, NEM a state-ből (mert az még üres lehet)
      const { data: meRow } = await supabase.from('staff').select('allowed_boards').eq('email', currentUser.email).maybeSingle()
      const allowed = meRow?.allowed_boards || []
      const first = (allowed.length ? data.find(b => allowed.includes(b.id)) : data[0]) || data[0]
      if (first) setActiveBoardId(first.id)
    } else if (!activeBoardId && data && data.length) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ height: 42, background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 6, padding: '0 14px', flexShrink: 0 }}>
            {(() => {
              const meRow = getMe()
              const allowed = meRow?.allowed_boards || []
              const visibleBoards = boards.filter(b => !allowed.length || allowed.includes(b.id))
              if (visibleBoards.length <= 1) {
                return <span style={{ fontSize: 13, fontWeight: 700 }}>{visibleBoards[0]?.name || 'ImmoPixels Board'}</span>
              }
              return (
