...
export async function generateZugferdPdf({ inv: invIn, items, seller, template = {}, clientKmTotal = 0 }) {
  ...
  ;(items || []).forEach((it, i) => {
    ...
    const rowH = Math.max(26, descLines.length * 11 + 18)
    ...
    y -= rowH
    // elválasztó vonal a tételek KÖZÉ, az utolsó szövegsortól jól elválasztva
    page.drawLine({ start: { x: L, y: y + 9 }, end: { x: R, y: y + 9 }, color: rgb(0.9, 0.88, 0.84) })
  })
