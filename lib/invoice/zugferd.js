...
export async function generateZugferdPdf({ inv, items, seller, template = {} }) {
  ...
  if (kmSum > 0) { T('Gefahrene Strecke gesamt: ' + kmSum.toLocaleString('de-DE') + ' km', L, y, 8, F, mut); y -= 16 }
  ...
  // ENTWURF-Wasserzeichen, solange keine Rechnungsnummer vergeben ist (Entwurf)
  if (!inv.invoice_number && !isStorno) {
    const wm = 'ENTWURF'; const wsize = 96
    pgs.forEach(pg => {
      pg.drawText(wm, { x: 88, y: PH / 2 - 40, size: wsize, font: B, color: rgb(0.85, 0.83, 0.79), rotate: degrees(45), opacity: 0.35 })
    })
  }
