      ...
      <div style={{ height: 52, background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', flexShrink: 0, boxShadow: 'var(--sh)', zIndex: 100 }}>
        ...
        <div className="desktop-nav" style={{ display: 'flex', alignItems: 'center', height: '100%', padding: '0 4px' }}>
          {[
            ...
            { id: 'amelia', label: 'Amelia', icon: 'ti-calendar-check' },
...
>
      ...
      {tab === 'staff' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: 22 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))', gap: 12 }}>
            {staff.map(s => (
              <div key={s.id}
                onClick={() => { if(me?.role_level==='admin'||me?.role_level==='subadmin') { setEditStaff(s); setStaffAvatarData(null); setModal('staff') } }}
                ...
                style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, textAlign: 'center', cursor: (me?.role_level==='admin'||me?.role_level==='subadmin') ? 'pointer' : 'default', boxShadow: 'var(--sh)', transition:'transform .18s cubic-bezier(.34,1.56,.64,1),box-shadow .18s,border-color .15s' }}>
                <div className="staff-av-circle" style={{ width: 64, height: 64, borderRadius: '50%', margin: '0 auto 10px', background: s.color + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, color: s.color, overflow: 'hidden', border: '2px solid transparent', transition: 'transform .22s cubic-bezier(.34,1.56,.64,1), border-color .15s' }}>
                  {s.avatar_url ? <img src={s.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : s.init}
                </div>
      ...
      {showWidgets && widgets.map(w => {
        ...
        return (
          <div key={w.id}
            style={{ position:'fixed', left:Math.min(w.x, typeof window!=='undefined'?window.innerWidth-w.w-10:9999), top:Math.max(0,Math.min(w.y, typeof window!=='undefined'?window.innerHeight-50:9999)), width:w.w, minWidth:200, minHeight: minimizedWidgets.includes(w.id)?0:120, background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:12, boxShadow:'var(--sh2)', zIndex:180, userSelect:'none', overflow:'hidden', resize: minimizedWidgets.includes(w.id)?'none':'both' }}
            onMouseDown={e => {
...
<form onSubmit={e => { e.preventDefault(); if(isAdminOrSub) saveStaffFn(new FormData(e.target)) }} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 13, padding: 22, width: 420, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.12)' }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 14 }}>
                <label style={{ cursor: 'pointer' }}>
                  <div style={{ width: 72, height: 72, borderRadius: '50%', margin: '0 auto 6px', background: (editStaff?._color||editStaff?.color||'#b8892a')+'22', border: '2px solid '+(editStaff?._color||editStaff?.color||'#b8892a'), display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', fontSize: 20, fontWeight: 700, color: editStaff?._color||editStaff?.color||'var(--t3)', transition:'border-color .2s,background .2s' }}>
