                    ...
                    {isCollapsed ? (
                      ...
                      <>
                        ...
                        <div style={{ overflowY: 'auto', padding: 8, display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
                          {colCards.map(card => (
                            <CardItem key={card.id} card={card} staff={staff} onlineUsers={onlineUsers} fontSize={fontSize} dragging={dragging} clients={clients} border={cardBorder(card)} onNoteChange={onNoteChange} onNoteEnter={onNoteEnter} onClick={() => setActiveCard(card)} onDragStart={() => setDragging(card)} onSend={openSend} droppedId={droppedCard}
                            onCheck={async (card) => {
...
function CardItem({ card, staff, border, onNoteChange, onNoteEnter, onClick, onDragStart, onSend, onCheck, onDelete, onColorChange, droppedId, noteMention, setNoteMention, dirtyCards, editingCards, onMoveUp, onMoveDown, onlineUsers, fontSize = 14, dragging = null, clients = [] }) {
  const t = TYPES[card.card_type] || TYPES.foto
