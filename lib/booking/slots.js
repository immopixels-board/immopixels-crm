...
async function busyIntervals(staffInit, dateStr) {
  // Resolve init → staff UUID (card_team only has staff_id)
  const { data: staffRow } = await sb()
    .from('staff').select('id').eq('init', staffInit).maybeSingle()
  if (!staffRow?.id) return []

  // Read ALL cards (GCal-synced + manual) for this staff on this date — no Google call
  const { data } = await sb()
    .from('cards')
    .select('id, card_time, booking_end_time, booking_service_id, card_date, ' +
            'card_team!inner(staff_id)')
    .eq('card_date', dateStr)
    .eq('card_team.staff_id', staffRow.id)
  ...
  return (data || []).map(c => {
    ...
    const buf = bufMap[c.booking_service_id] ?? 45
...
export async function getDaySlots(serviceId, dateStr, addonMin = 0) {
  const { data: svc } = await sb()
    .from('booking_services').select('*').eq('id', serviceId).single()
  if (!svc) return []

  const total = svc.duration_min + addonMin + svc.buffer_min // a slot ennyit foglal le
  const dur = svc.duration_min + addonMin                    // tényleges munkaidő
  ...
  const LAST_SLOT = 17 * 60 // utolsó foglalható kezdés 17:00, akkor is ha 18-ig tart a munkaidő
...
export async function pickLeastBusyProvider(candidateInits, dateStr) {
  ...
  // init → staff_id map
  const { data: staffRows } = await sb().from('staff').select('id, init').in('init', candidateInits)
  const idByInit = Object.fromEntries((staffRows || []).map(s => [s.init, s.id]))
  const counts = {}
  for (const init of candidateInits) {
    const sid = idByInit[init]
    if (!sid) { counts[init] = 0; continue }
    const { count } = await sb()
      .from('cards')
      .select('id, card_team!inner(staff_id)', { count: 'exact', head: true })
      .eq('card_date', dateStr)
      .eq('card_team.staff_id', sid)
    counts[init] = count || 0
  }
