...
export default function Home() {
  ...
  async function endCustomDrag() {
    ...
    if (card && dragDidMoveRef.current) {
      ...
      if (targetColId !== card.column_id && card.booking_source === 'online' && card.booking_status === 'pending' && card.booking_token) {
        ...
        if (!tgtTitle.includes('booking')) {
          try {
            const rc = await fetch('/api/booking/confirm', {
              ...
              body: JSON.stringify({ token: card.booking_token, confirmedByStaffId: me?.id })
            })
  ...
  async function loadBoards() {
    ...
    if (email) {
      const { data: row } = await supabase.from('staff').select('id,email,role_level,allowed_boards,init').eq('email', email).maybeSingle()
      meRow = row
      setMyRow(row)
    }
