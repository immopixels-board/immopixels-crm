...
export async function generateZugferdPdf({ inv: invIn, items, seller, template = {}, clientKmTotal = 0 }) {
  ...
  ;(items || []).forEach((it, i) => {
    ...
    const rowH = Math.max(22, descLines.length * 11 + 13)
    ...
    y -= rowH
    // elválasztó vonal a tételek KÖZÉ, az utolsó szövegsortól elegendő távolságra
    page.drawLine({ start: { x: L, y: y + 8 }, end: { x: R, y: y + 8 }, color: rgb(0.9, 0.88, 0.84) })
  })
