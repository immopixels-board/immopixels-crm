              ...
              {(() => {
                ...
                return (
                  <div key={col.id} className="bcol"
                    draggable={false}
                    style={{ width: isCollapsed ? 44 : 272, minWidth: isCollapsed ? 44 : 272, background: 'linear-gradient(' + getColStyle(col.color).bg + ',' + getColStyle(col.color).bg + '), var(--bg2)', border: '1px solid var(--border)', borderRadius: 11, display: 'flex', flexDirection: 'column', maxHeight: 'calc(100vh - 116px)', flexShrink: 0, transition: 'all .2s', cursor: isCollapsed ? 'pointer' : 'default', opacity: isColDragged ? 0.8 : 1 }}
                    data-colid={col.id}
>
                    {isCollapsed ? (
                      ...
                      <>
                        <div style={{ padding: '10px 12px 8px', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, background: colWidgetHeader?hexToGradient(col.dot_color||'#6b6b6e'):'none', borderRadius: colWidgetHeader?'10px 10px 0 0':0 }}>
                          <div
                            onMouseDown={e => {
                              e.preventDefault()
                              e.stopPropagation()
