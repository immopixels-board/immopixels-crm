...
const DANI_EXTRA_MIN = 15 // Dani (DB) lassabban dolgozik → ennyivel több időt foglalunk le neki / nagyobb buffer. Állítható.
...
export async function getDaySlots(serviceId, dateStr, addonMin = 0, excludeGcalId = null, shootAddress = null) {
  ...
  for (const p of providers) {
    ...
    // Dani (DB) lassabb → több időt foglalunk le neki és nagyobb buffert tartunk
    const pExtra = p.staff_init === 'DB' ? DANI_EXTRA_MIN : 0
