...
export async function generateZugferdPdf({ inv: invIn, items, seller, template = {}, clientKmTotal = 0 }) {
  const inv = { ...invIn, clientKmTotal: invIn.clientKmTotal ?? clientKmTotal }
  ...
  const wrap = (txt, f, size, maxW) => { const words = String(txt ?? '').split(/\s+/).filter(Boolean); const lines = []; let cur = ''; for (const w of words) { const t = cur ? cur + ' ' + w : w; if (cur && f.widthOfTextAtSize(t, size) > maxW) { lines.push(cur); cur = w } else cur = t } if (cur) lines.push(cur); return lines.length ? lines : [''] }

  // oszlop-horgonyok (jobbra zárt számok)
  const C = { pos: L, desc: L + 22, descMaxW: 272, anzahl: 360, preis: 422, rabatt: 466, steuer: 506, netto: R }
  ...
  // címzett — hosszú cégnév több sorba tördelve
  wrap(b.company || inv.client_name || '', B, 11, 300).forEach(ln => { T(ln, L, y, 11, B, dark); y -= 13 })
  if (b.contact) { T(b.contact, L, y, 9, F, dark); y -= 12 }
  ;(b.address || '').split(/[\n,]/).map(s2 => s2.trim()).filter(Boolean).forEach(line => { T(line, L, y, 9, F, dark); y -= 12 })
  y -= 16
  // sáv: bal "Rechnung <nr>", jobb Kundennummer / Datum (mint Billomatban)
  page.drawLine({ start: { x: L, y: y + 6 }, end: { x: R, y: y + 6 }, color: hair })
  T((isStorno ? 'Stornorechnung ' : 'Rechnung ') + (inv.invoice_number || 'Entwurf'), L, y - 4, 12, B, dark)
  const rLines = [b.kundennr ? 'Kundennummer: ' + b.kundennr : null, 'Datum: ' + fmtDate(inv.invoice_date)].filter(Boolean)
  rLines.forEach((ln, i) => RT(ln, R, y - 2 - i * 12, 9, F, mut))
  y -= (rLines.length > 1 ? 26 : 18)
  page.drawLine({ start: { x: L, y: y + 6 }, end: { x: R, y: y + 6 }, color: hair }); y -= 12
  ...
  ;(items || []).forEach((it, i) => {
    ...
    const rowH = Math.max(32, descLines.length * 11 + 26)
    if (y - rowH < 120) newPage(true)
    const net = it.line_net != null ? it.line_net : lineNet(it)
    T(String(i + 1), C.pos, y, 9, F, dark)
    descLines.forEach((dl, k) => T(trunc(dl, k === 0 ? B : F, k === 0 ? 8.5 : 8, C.descMaxW), C.desc, y - k * 11, k === 0 ? 8.5 : 8, k === 0 ? B : F, k === 0 ? dark : mut))
    RT(n2(it.qty) + (it.unit ? ' ' + it.unit : ''), C.anzahl, y, 9); RT(eur(it.unit_price), C.preis, y, 9)
    ...
    y -= rowH
    // elválasztó vonal a tételek KÖZÉ, mindkét tételtől jól elválasztva
    page.drawLine({ start: { x: L, y: y + 15 }, end: { x: R, y: y + 15 }, color: rgb(0.9, 0.88, 0.84) })
  })
  ...
  RT('USt 19 % (' + eur(inv.total_net) + '):', lblX, y, 9, F, mut); RT(eur(inv.vat_amount), C.netto, y, 9, F, dark); y -= 14
  page.drawLine({ start: { x: lblX - 10, y: y - 1 }, end: { x: R, y: y - 1 }, color: gold }); y -= 15
  RT('Gesamt brutto:', lblX, y, 11, B, dark); RT(eur(inv.total_gross), C.netto, y, 11, B, gold); y -= 24

  // Záró szövegblokk (Gefahrene Strecke / Dank / Zahlung / Online buchen) az oldal ALSÓ HARMADÁBA,
  // ha az utolsó oldal nincs tele. Ha tele van (y már alacsony), marad a tételek után.
  const closingTopMax = Math.round(PH * 0.34)
  if (y > closingTopMax) y = closingTopMax

  const kmSum = (items || []).reduce((s, it) => { if (it.unit === 'km') { const q = Number(it.qty) || 0; return s + q } const m = String(it.description || '').match(/([\d.,]+)\s*km/i); if (!m) return s; const n = parseFloat(m[1].replace(/\./g, '').replace(',', '.')); return s + (isNaN(n) ? 0 : n) }, 0)
  if (kmSum > 0) { T('Gefahrene Strecke (berechnet): ' + kmSum.toLocaleString('de-DE') + ' km', L, y, 8, F, mut); y -= 13 }
  if (inv.clientKmTotal > 0) { T('Wir haben für Ihre Aufträge insgesamt ' + Number(inv.clientKmTotal).toLocaleString('de-DE') + ' km zurückgelegt — überwiegend nicht berechnet.', L, y, 8, F, mut); y -= 13 }
  ...
  // Seite X von Y — minden oldal alján
  const pgs = pdf.getPages(); const total = pgs.length
  pgs.forEach((pg, i) => { const tx = 'Seite ' + (i + 1) + ' von ' + total; const w = F.widthOfTextAtSize(tx, 7); pg.drawText(tx, { x: (PW - w) / 2, y: 30, size: 7, font: F, color: mut }) })
