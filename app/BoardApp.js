...
import FahrtenWidget from '../components/FahrtenWidget'
...
export default function Home() {
  ...
  function buildInvoiceItem(c, group) {
    ...
    // vezető referencia-kód levágása: "KI#018 - ", "Riegel#061 - "
    addr = addr.replace(/^[A-Za-zÄÖÜäöüß0-9]*#\s*\d+\s*[-–—]\s*/i, '').trim()
    // kettőzött cím összevonása: "Str, HD, Str, Stadt" -> "Str, Stadt" (utolsó, teljes előfordulás)
    {
      const segs = addr.split(',').map(s => s.trim()).filter(Boolean)
      if (segs.length > 2) {
        const nrm = s => s.toLowerCase().replace(/[^a-z0-9äöüß]/g, '')
        const st = nrm(segs[0]); let li = -1
        for (let i = 1; i < segs.length; i++) { if (nrm(segs[i]) === st) li = i }
        if (li > 0) addr = segs.slice(li).join(', ')
      }
    }
    ...
    return { title: line1, description: parts.join(' + '), quantity: 1, unit_price: priceForCard(c, group.client), _date: c.card_date || '', _addr: addr }
  }
  ...
  // Auto-sync GCal: a board megnyitásakor (3s) és 5 percenként, ha a lap látható —
  // így a naptári változások (áthelyezett időpontok) maguktól megjelennek a kártyákon,
  // nem kell a "GCal" gombra kattintani.
  useEffect(() => {
    let stop = false
    const run = () => { if (!stop && typeof document !== 'undefined' && document.visibilityState === 'visible') syncGcal() }
...
>
      ...
      {showWidgets && tab === 'board' && widgets.map(w => {
        ...
        const TITLES = { weather:'Wetter', todo:'To Do', nexttermin:'Nächster Termin', stats:'Statistik', makler:'Makler / Monat', fahrten:'Fahrten' }
        const HEADGRAD = w.type==='weather'?'linear-gradient(135deg,#9fd5df,#7BBFCB)':w.type==='todo'?'linear-gradient(135deg,#dcc088,#C9A96E)':w.type==='nexttermin'?'linear-gradient(135deg,#b8cca8,#9CAF88)':w.type==='stats'?'linear-gradient(135deg,#ffd4b0,#FFBE98)':w.type==='makler'?'linear-gradient(135deg,#c9a96e,#6b6b6e)':w.type==='fahrten'?'linear-gradient(135deg,#8aa6c4,#5f7fa3)':'linear-gradient(135deg,#c49a75,#A67B5B)'
        ...
        return (
          <div key={w.id}
            ...
            <div style={{ padding:'9px 12px', background: headBg, borderBottom:'none', display:'flex', alignItems:'center', gap:7, cursor:'grab', opacity: minimizedWidgets.includes(w.id) ? 0.5 : 1, transition:'opacity .2s', position:'relative' }}>
              <i className={'ti ' + (w.type==='weather'?'ti-cloud':w.type==='todo'?'ti-check':w.type==='nexttermin'?'ti-calendar':w.type==='stats'?'ti-chart-bar':w.type==='makler'?'ti-building-community':w.type==='fahrten'?'ti-car':'ti-layout-grid')} style={{ fontSize:15, color: headFg }} />
        ...
        <div style={{ position:'fixed', bottom:50, left:16, display:'flex', gap:6, zIndex:180, flexWrap:'wrap', maxWidth:600 }}>
          {widgets.filter(w=>!w.open && !w.disabled).filter(w=> !(w.type==='makler' && me?.role_level!=='admin')).map(w => (
            <button key={w.id} onClick={() => saveWidgets(widgets.map(ww=>ww.id===w.id?{...ww,open:true}:ww))}
              ...
              {w.type==='weather'?'🌤️':w.type==='todo'?'✅':w.type==='nexttermin'?'📅':w.type==='stats'?'📊':w.type==='makler'?'📋':w.type==='fahrten'?'🚗':w.title||'🧩'} öffnen
            </button>
          ))}
        </div>
      )}
      ...
      {customWidgetModal && (
        <div onClick={e=>{if(e.target===e.currentTarget)setCustomWidgetModal(false)}} style={{ position:'fixed', inset:0, background:'rgba(28,26,22,.4)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(3px)' }}>
          <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:13, padding:22, width:480, maxWidth:'95vw', boxShadow:'0 20px 60px rgba(0,0,0,.12)' }}>
            ...
            <button onClick={()=>{
              const exists = widgets.find(w=>w.type==='fahrten')
              if (exists) saveWidgets(widgets.map(w=>w.type==='fahrten'?{...w,open:true,disabled:false}:w))
              else saveWidgets([...widgets, { id:'fahrten', type:'fahrten', x:140, y:110, w:320, h:380, open:true }])
              setCustomWidgetModal(false)
            }} style={{ width:'100%', display:'flex', alignItems:'center', gap:8, padding:'10px 12px', marginBottom:14, background:'var(--gdbg)', border:'1px solid var(--gold)', borderRadius:9, cursor:'pointer', color:'var(--gold)', fontSize:13, fontWeight:700 }}>
              <i className="ti ti-car" style={{ fontSize:16 }} /> Fahrten-Übersicht hinzufügen (km pro Fotograf)
            </button>
              ...
              <div style={{ marginBottom:14 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:6, marginTop:6 }}>
                  {widgets.filter(w=> !(w.type==='makler' && me?.role_level!=='admin')).map(w => {
                    const on = !w.disabled
                    const label = w.title || ({weather:'Wetter',todo:'To Do',nexttermin:'Nächster Termin',stats:'Statistik',makler:'Makler / Monat',fahrten:'Fahrten'}[w.type]) || w.type
                    return (
