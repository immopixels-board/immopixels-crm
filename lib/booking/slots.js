...
// per-fotós extra buffer: a booking_providers.extra_buffer_min mezőből jön (admin UI-ban állítható)
...
// ha a naptári eseményen nincs location, a CÍMBŐL (summary) próbáljuk: "[STATUS] Kunde - Adresse"
function addrFromSummary(s) {
  const t = String(s || '').replace(/^\s*\[[^\]]+\]\s*/, '').trim() // [AUSSTEHEND]/[BESTÄTIGT] levágása
  const i = t.indexOf(' - ')
  if (i < 0) return null
  const cand = t.slice(i + 3).trim()
  if (!cand || !/[\d,]/.test(cand)) return null // legyen benne szám vagy vessző (cím-jelleg)
  return cand
}
function toHHMM(min) {
...
async function getGoogleToken() {
  ...
  const demoStaffId = process.env.DEMO_BOOKING_STAFF_ID
  let refreshRow = null
  if (demoStaffId) {
    const { data } = await sb().from('gcal_tokens').select('refresh_token').eq('staff_id', demoStaffId).maybeSingle()
    refreshRow = data
  } else if (process.env.NEXT_PUBLIC_IS_DEMO === '1' || process.env.DEMO_CAL_ID) {
    // Demo: az első elérhető refresh_token (a demo-fiók összekötése)
    const { data } = await sb().from('gcal_tokens').select('refresh_token').not('refresh_token', 'is', null).limit(1).maybeSingle()
    refreshRow = data
  } else {
    const { data } = await sb().from('gcal_tokens').select('refresh_token').eq('staff_id', CD_STAFF_ID).maybeSingle()
    refreshRow = data
  }
  const data = refreshRow
...
async function busyIntervals(staffInit, dateStr, token, excludeGcalId = null) {
  ...
  for (const ev of items) {
    ...
    intervals.push({ start: startMin, end: endMin, location: ev.location || addrFromSummary(ev.summary) || null, gcalId: ev.id || null })
  }
  return intervals
}
...
async function providersForService(serviceId) {
  const { data } = await sb()
    ...
    .select('provider_id, booking_providers!inner(id, staff_init, name, active, extra_buffer_min)')
...
export async function getDaySlots(serviceId, dateStr, addonMin = 0, excludeGcalId = null, shootAddress = null) {
  ...
  for (const p of providers) {
    ...
    // Helyszín pótlása a CRM-kártyából: ha a naptári eseményen nincs location, de van hozzá
    // foglalás-kártya (gcal_id), abból vesszük a booking_address-t. Így a rendszer az ELŐZŐ
    // termin VALÓS helyéről számol (pl. Landau), nem feltételez tévesen otthont.
    try {
      const need = busy.filter(b => !b.location && b.gcalId)
      if (need.length) {
        const ids = [...new Set(need.map(b => b.gcalId))]
        const { data: locCards } = await sb().from('cards').select('gcal_id, booking_address, addr').in('gcal_id', ids)
        const byId = Object.fromEntries((locCards || []).map(c => [c.gcal_id, c.booking_address || c.addr || null]))
        for (const b of busy) { if (!b.location && b.gcalId && byId[b.gcalId]) b.location = byId[b.gcalId] }
      }
    } catch { /* ha nem sikerül, marad a konzervatív ág lentebb */ }
    // időrendben, hogy a szomszédos eseményeket megtaláljuk
    const sorted = [...busy].sort((a, b) => a.start - b.start)

    // per-fotós extra buffer (admin UI-ban állítható, booking_providers.extra_buffer_min)
    const pExtra = Number(p.extra_buffer_min) || 0
    const pDur = dur + pExtra
    const pTotal = total + pExtra

    for (const w of wd) {
      const wStart = toMin(w.start_time), wEnd = toMin(w.end_time)
      for (let t = wStart; t + pDur <= wEnd && t <= LAST_SLOT; t += SLOT_STEP_MIN) {
        if (isToday && t <= nowMin + 60) continue
        const free = !busy.some(b => overlaps(t, t + pTotal, b.start, b.end))
        if (!free) continue
        ...
        if (shootAddress) {
          ...
          if (prev?.location) {
            ...
            // VAN előző termin aznap, de ismeretlen a helye (sem naptár-location, sem CRM-cím)
            // → NEM feltételezünk otthont (az hibás "otthonról indul" eset). Óvatosan jelöljük.
            tight = true
            info = { from: 'unbekannt', home: false, prevEnd: toHHMM(prev.end), travelMin: null, eta: null, delayMin: 0, originQuery: null, unknownPrev: true }
          }
          if (!blocked && next?.location) {
            const { from } = await travelPair(next.location)
            if (from != null) {
              const gap = next.start - (t + pDur)
