...
const AUTO_CL = ['Fotografiert', 'In Bearbeitung', 'Rausgeschickt']
...
const DAYS = { Mon:'H', Tue:'K', Wed:'Sze', Thu:'Cs', Fri:'P', Sat:'Szo', Sun:'V' }
function fmtDate(d) {
  if (!d) return ''
...
const LS = { fontSize:10, fontWeight:700, color:'var(--t3)', textTransform:'uppercase', letterSpacing:'.5px', marginBottom:4, display:'block' }
const IS = { background:'var(--bg3)', border:'1.5px solid var(--border)', borderRadius:7, padding:'7px 10px', fontSize:13, color:'var(--t1)', fontFamily:'Arial', outline:'none', width:'100%' }
const BTNP = { background:'var(--gold)', color:'#fff', border:'none', borderRadius:7, padding:'7px 14px', fontSize:13, fontWeight:700, cursor:'pointer' }
const BTNG = { background:'none', border:'1.5px solid var(--brd2)', color:'var(--t2)', borderRadius:7, padding:'7px 14px', fontSize:13, fontWeight:700, cursor:'pointer' }
const BTNR = { background:'none', border:'1.5px solid var(--rdbr)', color:'var(--red)', borderRadius:7, padding:'7px 14px', fontSize:13, fontWeight:700, cursor:'pointer' }
...
export default function Home() {
  ...
  async function moveCardInColumn(card, direction) {
    ...
    const swap = colCards[swapIdx]
    const posA = card.position ?? idx
    const posB = swap.position ?? swapIdx
    setCards(p => p.map(c => {
      if (c.id === card.id) return { ...c, position: posB }
      if (c.id === swap.id) return { ...c, position: posA }
      return c
    }))
    await Promise.all([
      supabase.from('cards').update({ position: posB }).eq('id', card.id),
      supabase.from('cards').update({ position: posA }).eq('id', swap.id),
    ])
  ...
  async function loadAll() {
    setLoading(true)
    await Promise.all([loadCols(), loadCards(), loadStaff(), loadClients(), loadMaklers(), loadSettings(), loadServicePrices()])
    setLoading(false)
  ...
  function cardBorder(card) {
    if (!card.card_date || card.is_todo) return '1px solid var(--border)'
    const shootCol = cols.find(c => c.title === 'Shootings')
    if (!shootCol || card.column_id !== shootCol.id) return '1px solid var(--border)'
    const diffH = (new Date() - new Date(card.card_date + 'T00:00:00')) / 3600000
    if (diffH > 48) return '2px solid #b91c1c'
    if (diffH > 0) return '2px solid #f97316'
  ...
  function exportClients() {
    const rows = [['Firmenname', 'Kürzel', 'Adresse', 'E-Mail', 'Telefon', 'USt-ID', 'Kategorie', 'Makler', 'E-Mail', 'Tel']]
    clients.forEach(c => rows.push([c.name, c.short_name || '', c.addr || '', c.email || '', c.tel || '', c.vat || '', c.category || '', c.contact_name || '', c.contact_email || '', c.contact_tel || '']))
    const csv = rows.map(r => r.map(x => '"' + String(x).replace(/"/g, '""') + '"').join(',')).join('\n')
    const a = document.createElement('a')
    a.href = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv)
    a.download = 'ugyfelek.csv'
    a.click()
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ height: 52, background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', flexShrink: 0, boxShadow: 'var(--sh)', zIndex: 100 }}>
        ...
        <div className="desktop-nav" style={{ display: 'flex', alignItems: 'center', height: '100%', padding: '0 4px' }}>
          ...
          <div onClick={() => setAiOpen(p => !p)} style={{ padding: '0 13px', height: '100%', display: 'flex', alignItems: 'center', gap:6, fontSize: 12, fontWeight: 600, color: aiOpen ? 'var(--gold)' : 'var(--t3)', cursor: 'pointer', borderBottom: aiOpen ? '2px solid var(--gold)' : '2px solid transparent' }}>
      ...
      {showDebug && (
        <div style={{ position: 'fixed', top: 52, right: 0, width: 400, height: 'calc(100vh - 52px)', background: 'var(--bg2)', borderLeft: '1px solid var(--border)', boxShadow: 'var(--sh2)', zIndex: 150, display: 'flex', flexDirection: 'column', fontFamily: 'monospace', fontSize: 11 }}>
          ...
          <DebugPanel supabase={supabase} localLog={debugLog} />
        </div>
      )}
                        ...
                        <div style={{ overflowY: 'auto', padding: 8, display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
                          {colCards.map(card => (
                            ...
                            onDelete={async (card) => {
                              ...
                              await supabase.from('card_team').delete().eq('card_id',card.id)
                              await supabase.from('checklist_items').delete().eq('card_id',card.id)
                              await supabase.from('cards').delete().eq('id',card.id)
                              loadCards(); addLog('Gelöscht: '+card.title)
                            }}
      ...
      {/* ── CALENDAR ── */}
      {tab === 'gcal' && (
        <GoogleCalendarView staff={staff} me={me} supabase={supabase} />
      )}
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid var(--border)', background: 'var(--bg2)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '12px 14px 10px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              ...
              <button onClick={() => setModal('new-card')} className='btn-primary-anim' style={{ ...BTNP, width: '100%', padding: '6px', fontSize: 12, justifyContent: 'center', display: 'flex' }}>+ Neue Karte</button>
            </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 7 }}>
              ...
              <button onClick={() => { setEditClient(null); setClientColorState(''); setModal('client') }} className='btn-primary-anim' style={{ display: (me?.role_level==='admin'||me?.role_level==='subadmin') ? '' : 'none' }} style={{ ...BTNP, padding: '6px 12px', fontSize: 12 }}>+ Neuer Kunde</button>
            </div>
          </div>
      ...
      {tab === 'staff' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            ...
            <button onClick={() => { setEditStaff(null); setStaffAvatarData(null); setModal('staff') }} className='btn-primary-anim' style={{ display: (me?.role_level==='admin'||me?.role_level==='subadmin') ? '' : 'none' }} style={{ ...BTNP, padding: '6px 12px', fontSize: 12 }}>+ Mitarbeiter</button>
          </div>
      ...
      {activeCard && (
        ...
        />
            ...
            {colModal && (
        <ColumnModal col={colModal} onClose={() => setColModal(null)} isAdmin={me?.role_level==='admin'}
          onSave={async (name, colorKey, privateCol) => {
            ...
            if (colModal.id) {
              ...
              await supabase.from('columns').insert({ title: name, color: colorKey || null, position: (mx?.[0]?.position || 0) + 1 })
            }
        ...
        <div onClick={e => { if (e.target === e.currentTarget && modal !== 'client' && modal !== 'staff') setModal(null) }} style={{ position: 'fixed', inset: 0, background: 'rgba(28,26,22,.4)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
          ...
          {modal === 'new-card' && (
            <form onSubmit={async e => { e.preventDefault(); if(submitting) return; setSubmitting(true); await createCard(new FormData(e.target)); setSubmitting(false); broadcastChange('cards_changed') }} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 13, padding: 22, width: 480, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.12)' }}>
              ...
              <div style={{ marginBottom: 9 }}>
                ...
                <input id="nc-title" name="title" required placeholder="z.B. Müller - Hauptstr. 5" style={IS} />
              </div>
              {cardType !== 'todo' && (
                <>
                  <div style={{ marginBottom: 9 }}>
          ...
          {modal === 'client' && (
            <form onSubmit={e => { e.preventDefault(); saveClient(new FormData(e.target)) }} className="modal-animate" style={{ background: 'var(--bg2)', border: '0.5px solid var(--border)', borderRadius: 13, padding: 22, width: 500, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.12)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginBottom: 9 }}>
                <div><label style={LS}>Email</label><input name="email" type="email" defaultValue={editClient?.email || ''} style={IS} /></div>
                <div><label style={LS}>Telefon</label><input name="tel" defaultValue={editClient?.tel || ''} style={IS} /></div>
              </div>
  ...
  return (
    <div
      ...
      <div className="card-hover-menu" onClick={e=>e.stopPropagation()} style={{ position:'absolute', top:6, right:6, background:'var(--bg2)', border:'0.5px solid var(--border)', borderRadius:8, boxShadow:'0 2px 8px rgba(0,0,0,.08)', padding:'4px', display:'flex', alignItems:'center', gap:3, zIndex:10 }}>
        <div title="Verschieben" draggable
          onDragStart={e => { e.stopPropagation(); onDragStart(); e.currentTarget.closest('.board-card').classList.add('dragging-active'); setTimeout(()=>e.currentTarget.closest('.board-card').classList.add('drag-placeholder'),0) }}
          style={{ padding:'4px 5px', borderRadius:5, color:'var(--t2)', display:'flex', alignItems:'center', cursor:'grab', transition:'background .12s,color .12s' }}
          onMouseEnter={e=>{e.currentTarget.style.background='rgba(184,137,42,.08)';e.currentTarget.style.color='var(--gold)'}}
          onMouseLeave={e=>{e.currentTarget.style.background='none';e.currentTarget.style.color='var(--t2)'}}>
          <i className="ti ti-arrows-move" style={{ fontSize:13 }}></i>
        </div>
      ...
      <div draggable
        onDragStart={e => { e.stopPropagation(); onDragStart(); e.currentTarget.closest('.board-card').classList.add('dragging-active'); setTimeout(()=>e.currentTarget.closest('.board-card').classList.add('drag-placeholder'),0) }}
        className='card-title-main card-drag-handle'
