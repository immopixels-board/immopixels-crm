...
export default function Home() {
  ...
  async function loadBoards() {
    ...
    setBoards(data || [])
    // myRow betöltése (a Daniel-háttérhez kell az init)
    ...
    // mindenki minden boardot lát, alapból az elsőt választjuk
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div style={{ height: 42, background: 'var(--bg2)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 6, padding: '0 14px', flexShrink: 0 }}>
            {(() => {
              ...
              if (boards.length === 1) {
                return <span style={{ fontSize: 13, fontWeight: 700 }}>{boards[0].name}</span>
              }
              return (
