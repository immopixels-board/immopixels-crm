...
>
      ...
      {showWidgets && tab === 'board' && widgets.map(w => {
        ...
        return (
          <div key={w.id}
            ...
            <div style={{ padding:'9px 12px', background: w.type==='weather' ? 'linear-gradient(135deg,#9fd5df,#7BBFCB)' : w.type==='todo' ? 'linear-gradient(135deg,#dcc088,#C9A96E)' : w.type==='nexttermin' ? 'linear-gradient(135deg,#b8cca8,#9CAF88)' : w.type==='stats' ? 'linear-gradient(135deg,#ffd4b0,#FFBE98)' : w.type==='makler' ? 'linear-gradient(135deg,#c9a96e,#b8892a)' : 'linear-gradient(135deg,#c49a75,#A67B5B)', borderBottom:'none', display:'flex', alignItems:'center', gap:7, cursor:'grab', opacity: minimizedWidgets.includes(w.id) ? 0.5 : 1, transition:'opacity .2s' }}>
              <i className={'ti ' + (w.type==='weather'?'ti-cloud':w.type==='todo'?'ti-check':w.type==='nexttermin'?'ti-calendar':w.type==='stats'?'ti-chart-bar':w.type==='makler'?'ti-building-community':'ti-layout-grid')} style={{ fontSize:15, color: w.type==='stats'?'#7a3a10':'#fff' }} />
            ...
            <div className="widget-content" style={{ padding:12, display: minimizedWidgets.includes(w.id) ? ("none") : ("block") }}>
              ...
              {w.type==='makler' && (() => {
                ...
                cards.forEach(c => {
                  ...
                  const name = ((c.client_name||'').trim()) || ((c.customer_name||'').trim()) || ((c.title||'').trim()) || '—'
                ...
                if (!months.length) return <div style={{ color:'var(--t3)', fontSize:12, padding:'4px 2px' }}>Noch keine Aufnahmen.<br/><span style={{ fontSize:10 }}>({cards.filter(c=>!c.is_todo&&c.card_type!=='todo').length} Karten geladen)</span></div>
                return (
                  <div>
                    {months.map(m => {
                      const ms = maklerData[m] || { checked:[], hidden:[], extra:[], rename:{} }
                      const open = (m in openMap) ? openMap[m] : (m === months[0])
      ...
      {showWidgets && tab === 'board' && widgets.some(w => !w.open) && (
        <div style={{ position:'fixed', bottom:50, left:16, display:'flex', gap:6, zIndex:180, flexWrap:'wrap', maxWidth:600 }}>
          {widgets.filter(w=>!w.open).filter(w=> !(w.type==='makler' && me?.role_level!=='admin')).map(w => (
          ...
          <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:13, padding:22, width:480, maxWidth:'95vw', boxShadow:'0 20px 60px rgba(0,0,0,.12)' }}>
            ...
            {me?.role_level==='admin' && (
              <button onClick={()=>{
                ...
                else saveWidgets([...widgets, { id:'makler', type:'makler', x:120, y:100, w:300, h:380, open:true }])
