          ...
          <div key={w.id}
            ...
            <div className="widget-content" style={{ padding:12, display: minimizedWidgets.includes(w.id) ? ("none") : ("block") }}>
              ...
              {w.type==='makler' && (() => {
                ...
                const byMonth = {}
                cards.forEach(c => {
                  if (c.is_todo || c.card_type==='todo' || !c.card_date) return
                  const mk = c.card_date.slice(0,7)
                ...
                if (!months.length) return <div style={{ color:'var(--t3)', fontSize:12, padding:'4px 2px' }}>Noch keine Aufnahmen.</div>
                return (
                  <div>
                    {months.map(m => {
