        ...
        {notifications.map(n => {
          ...
          return (
            <div key={n.id} onClick={() => onRead(n)} style={{ padding:'10px 14px', display:'flex', gap:9, alignItems:'flex-start', borderBottom:'0.5px solid var(--border)', cursor:'pointer', background: n.read ? 'none' : 'rgba(184,137,42,.04)', transition:'background .12s' }}
              ...
              <div style={{ width:30, height:30, borderRadius:'50%', background:(sender?.color||'#6b6b6e')+'22', color:sender?.color||'var(--gold)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, flexShrink:0, overflow:'hidden' }}>
